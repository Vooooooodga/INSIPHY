...
def unchanged_protein_alignment(query, target, threads=1):
...
class CodingCorrespondenceTests(unittest.TestCase):
    ...
    def test_split_codon_utr_and_negative_strand_exact_projection(self):
        for strand in ("+", "-"):
            with self.subTest(strand=strand), patch(
                "insiphy.coding_correspondence.alignment.protein_pair_alignment",
                ...
                self.assertEqual(left["protein_projected_blocks"], ((1, 4, 3, 6),))
                self.assertEqual(right["protein_projected_blocks"], ((1, 5, 7, 11),))
                self.assertEqual(reverse["protein_projected_blocks"], ((7, 11, 1, 5),))
                ...
                self.assertEqual(aligner.call_args.args, ("MEA", "MEA"))
    ...
    def test_internal_stops_unknown_codons_and_protein_gaps_are_unmapped(self):
        ...
        with patch("insiphy.coding_correspondence.alignment.protein_pair_alignment", return_value=("MXXAA", "ME-AA")):
        ...
        self.assertEqual(evidence["protein_projected_blocks"], ((1, 3, 1, 3), (10, 15, 7, 12)))
    ...
    def test_disagreeing_supported_isoform_coordinates_remain_ambiguous(self):
        ...
        with patch("insiphy.coding_correspondence.alignment.protein_pair_alignment", side_effect=unchanged_protein_alignment):
            evidence = index.evidence("q", "t")
    ...
    def test_aa_identity_gate_does_not_use_dna_threshold_or_context(self):
        ...
        self.assertEqual(evidence["protein_status"], "low_protein_similarity_or_coverage")
    ...
    def test_metrics_name_best_pair_while_coordinates_include_compatible_pairs(self):
        ...
        def alignment_by_transcript(query, target, threads=1):
            self.assertEqual(target, "MAAA")
            return {"MAA": "MAA-", "AAA": "-AAA"}[query], target

        with patch("insiphy.coding_correspondence.alignment.protein_pair_alignment", side_effect=alignment_by_transcript):
            evidence = index.evidence("q", "t")
        self.assertEqual(evidence["protein_projected_blocks"], ((1, 12, 1, 12),))
    ...
    def test_protein_blocks_enter_graph_and_junction_preserving_raw_dna(self):
        for strand in ("+", "-"):
            with self.subTest(strand=strand), patch(
                ...
                occurrences, paths, sequences = self.split_fixture(strand)
                homology, matches = cluster_segments(occurrences, sequences, transcript_paths=paths)
    ...
    def test_repeated_reference_coverage_does_not_merge_distinct_exons(self):
        ...
        with patch("insiphy.preprocess.match_evidence", side_effect=weak_dna_evidence), patch(
            "insiphy.coding_correspondence.alignment.protein_pair_alignment", side_effect=unchanged_protein_alignment,
        ):
    ...
    def test_unavailable_metadata_preserves_dna_and_internal_backend_never_calls_protein(self):
        ...
        for backend, expected in (("mafft", "unavailable"), ("internal", "disabled_backend")):
            with self.subTest(backend=backend), patch(
                "insiphy.preprocess.match_evidence", side_effect=weak_dna_evidence,
            ), patch("insiphy.coding_correspondence.alignment.protein_pair_alignment") as aligner:
    ...
    def test_external_aligner_error_propagates(self):
        occurrences, paths, sequences = self.split_fixture()
        with patch("insiphy.preprocess.match_evidence", side_effect=weak_dna_evidence), patch(
            "insiphy.coding_correspondence.alignment.protein_pair_alignment", side_effect=AlignmentBackendError("explicit MAFFT failure"),
        ):
